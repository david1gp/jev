...
repository="david1gp/jev"

for command_name in gh jq git-cliff sed; do
...
if [[ "$(git branch --show-current)" != "main" ]]; then
  printf 'Error: releases must be prepared from the main branch.\n' >&2
  exit 1
fi

if [[ -n "$(git status --porcelain)" ]]; then
  printf 'Error: the worktree must be clean before preparing a release.\n' >&2
  exit 1
fi
